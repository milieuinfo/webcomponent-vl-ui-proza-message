...
describe('vl-proza-message', async () => {
  ...
  it('als gebruiker zie ik de parameter keys in bewerk modus', async () => {
    const message = await vlProzaMessagePage.getMessageWithParameters();
    await message.edit();
    await assert.eventually.equal(message.getWysiwygText(), 'Lorem ${this.key2} dolor sit amet, consectetur adipiscing elit. Duis iaculis molestie feugiat. Lorem ${this.key2} eros, consequat et venenatis ac, scelerisque feugiat nunc. Nam molestie tincidunt lectus, nec volutpat ante egestas at. Curabitur quis odio metus. Morbi at purus ac purus convallis ${this.key1} at eu est. Nunc id ligula quis justo semper ullamcorper. Donec orci nisi, ${this.key1} varius massa ut, vulputate imperdiet nibh. Maecenas ${this.key1} lectus quis turpis cursus, ac vehicula ligula fermentum.\n\nPraesent consequat diam nec semper congue. ${this.key2} tempor ut erat nec aliquam. Quisque ullamcorper sapien magna, sit amet porta ${this.key2} pulvinar aliquam. Sed eleifend fringilla augue in vehicula. Sed leo sem, imperdiet non ornare maximus, bibendum facilisis massa. Nunc condimentum leo mi, quis porta ante mattis ut. Quisque eu enim vel metus consequat iaculis. Donec malesuada odio quis quam vulputate vestibulum.');
  });
